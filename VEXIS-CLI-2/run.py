...
def main():
    ...
    if len(sys.argv) < 2 and not any(flag in sys.argv for flag in ["--install-sdks", "--sdk-status", "--help"]):
        ...
        print("  --debug           Enable debug mode")
        print("  --no-prompt       Use saved provider preference without prompting")
        print("  --provider        Specify AI provider (ollama, google, openai, etc.)")
        print("  --model           Specify model name for the provider")
    ...
    # Model selection - check for command line args first
    selected_provider = None
    selected_model = None
    
    # Parse --provider and --model arguments
    if "--provider" in sys.argv:
        try:
            idx = sys.argv.index("--provider")
            if idx + 1 < len(sys.argv) and not sys.argv[idx + 1].startswith("--"):
                selected_provider = sys.argv[idx + 1]
    ...
    # If provider/model specified via command line, use them
    if selected_provider:
        print(f"\nUsing provider: {selected_provider}")
        if selected_model:
            print(f"Using model: {selected_model}")
    elif "--no-prompt" not in sys.argv:
